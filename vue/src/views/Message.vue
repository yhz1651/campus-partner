...
const confirmDelBatch = () => {
  ...
  request.post('/message/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/message/export`)
}
...
<template>
  <div>
    <div>
      <el-input v-model="name" placeholder="请输入通知内容" class="w300" />
    ...
    <div style="margin: 10px 0">
      <el-upload
          v-if="auths.includes('message.import')"
          class="ml5"
          :show-file-list="false"
          style="display: inline-block; position: relative; top: 3px"
          :action='`http://${config.serverUrl}/message/import`'
      ...
      <el-button type="primary" @click="exportData" class="ml5" v-if="auths.includes('message.export')">
        <el-icon style="vertical-align: middle">
          <Top />
        </el-icon>  <span style="vertical-align: middle"> 导出 </span>
      </el-button>
      <el-popconfirm title="您确定删除吗？" @confirm="confirmDelBatch" v-if="auths.includes('message.deleteBatch')">
        <template #reference>
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column label="收到通知的用户"><template #default="scope"><span v-if="scope.row.userId">{{ state.userOptions.find(v => v.id === scope.row.userId) ? state.userOptions.find(v => v.id === scope.row.userId).name : '' }}</span></template></el-table-column>

        <el-table-column label="操作" width="180">
          <template #default="scope">
<!--            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('message.edit')">编辑</el-button>-->
            <el-popconfirm title="您确定删除吗？" @confirm="del(scope.row.id)" v-if="auths.includes('message.delete')">
              <template #reference>
...
<!--    <el-dialog v-model="dialogFormVisible" title="消息信息" width="40%">-->
