...
const confirmDelBatch = () => {
  ...
  request.post('/comment/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/comment/export`)
}
...
<template>
  <div>
    <div>
      <el-input v-model="name" placeholder="请输入评论内容" class="w300"/>
...
<!--      <el-button type="success" @click="handleAdd" v-if="auths.includes('comment.add')">-->
<!--        <el-icon style="vertical-align: middle">-->
<!--          <Plus/>-->
<!--        </el-icon>-->
...
<!--      </el-button>-->
      <el-upload
          v-if="auths.includes('comment.import')"
          class="ml5"
          :show-file-list="false"
          style="display: inline-block; position: relative; top: 3px"
          :action='`http://${config.serverUrl}/comment/import`'
      ...
      <el-button type="primary" @click="exportData" class="ml5" v-if="auths.includes('comment.export')">
        <el-icon style="vertical-align: middle">
          <Top/>
        </el-icon>
        <span style="vertical-align: middle"> 导出 </span>
      </el-button>
      <el-popconfirm title="您确定删除吗？" @confirm="confirmDelBatch" v-if="auths.includes('comment.deleteBatch')">
        <template #reference>
...
<!--        <el-table-column prop="dynamicId" label="动态id"></el-table-column>-->
        ...
        <el-table-column prop="location" label="用户属地"></el-table-column>
<!--        <el-table-column prop="pid" label="父级用户id"></el-table-column>-->
        <el-table-column label="父级用户">
          <template #default="scope">
            <span v-if="scope.row.puserId">{{ state.userOptions.find(v => v.id === scope.row.puserId) ? state.userOptions.find(v => v.id === scope.row.puserId).name : '' }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="180">
          <template #default="scope">
<!--            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('comment.edit')">编辑</el-button>-->
            <el-popconfirm title="您确定删除吗？" @confirm="del(scope.row.id)" v-if="auths.includes('comment.delete')">
              <template #reference>
    ...
    <el-dialog v-model="dialogFormVisible" title="评论信息" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        ...
        <el-form-item prop="location" label="属地">
...
<!--        </el-form-item>-->
        <el-form-item prop="puserId" label="父级用户">
          <el-select clearable v-model="state.form.puserId" placeholder="请选择" style="width: 100%">
