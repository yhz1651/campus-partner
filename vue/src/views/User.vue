...
<template>
  <div>
    <div>
      <el-input v-model="username" placeholder="请输入用户名" class="w300" />
      <el-input v-model="name" placeholder="请输入姓名" class="w300 ml5" />
      ...
      <el-table :data="state.tableData" stripe border  @selection-change="handleSelectionChange">
        ...
        <el-table-column label="头像" width="120">
          <template #default="scope">
            <el-image preview-teleported style="width: 80px; height: 80px" :src="scope.row.avatar" v-if="scope.row.avatar" :preview-src-list="[scope.row.avatar]"></el-image>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="status" label="是否禁用" >
          <template #default="scope">
            <el-switch v-if="scope.row.role !== 'ADMIN'" v-model="scope.row.status" @change="changeStatus(scope.row)"></el-switch>
            <el-tag type="danger" v-else> 无法禁用 </el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogFormVisible" title="用户信息" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        <el-form-item prop="username" label="用户名" v-if="!state.form.id">
          <el-input v-model="state.form.username" autocomplete="off" />
        </el-form-item>
        <el-form-item prop="username" label="用户名" v-else>
          <el-input v-model="state.form.username" disabled autocomplete="off" />
        </el-form-item>
        <el-form-item prop="email" label="邮箱" v-if="!state.form.id">
          <el-input v-model="state.form.email" autocomplete="off" />
        </el-form-item>
        <el-form-item prop="email" label="邮箱" v-else>
          <el-input v-model="state.form.email" disabled autocomplete="off" />
        </el-form-item>
