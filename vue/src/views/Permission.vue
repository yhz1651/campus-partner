...
<!--        <el-table-column prop="pid" label="父级id"></el-table-column>-->
        ...
        <el-table-column prop="hide" label="是否隐藏">
          <template #default="scope" >
            <el-switch v-if="scope.row.type !== 3" v-model="scope.row.hide" @change="changeHide(scope.row)"></el-switch>
            <div v-else></div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="180">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('permission.edit')">编辑</el-button>
            <el-popconfirm title="您确定删除吗？" @confirm="del(scope.row.id)" v-if="auths.includes('permission.delete')">
              <template #reference>
    ...
    <el-dialog v-model="dialogFormVisible" title="权限信息" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        ...
        <el-form-item prop="icon" label="图标"  v-if="state.form.type === 1 || state.form.type === 2">
          <el-select v-model="state.form.icon" filterable placeholder="请选择" style="width: 100%">
            <el-option
                v-for="item in icons"
                :key="item.id"
                :label="item.code"
